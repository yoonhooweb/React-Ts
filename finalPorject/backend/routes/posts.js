...
router.get("/", async (req, res) => {
    try {
        ...
        // 6) 실제 조회 쿼리
        const query = Post.find(filter).populate("author", "nickname profileImage").populate("comments.author", "nickname profileImage").lean();
...
router.get("/overview", async (req, res) => {
    try {
        ...
        // 4) 최신 6개
        const latestPosts = await Post.find().sort({ createdAt: -1 }).limit(6).populate("author", "nickname profileImage").lean();

        // 5) 인기 6개 (viewCount 순)
        const popularPosts = await Post.find().sort({ viewCount: -1 }).limit(6).populate("author", "nickname profileImage").lean();
...
router.delete("/:postId/comments/:commentId", auth, async (req, res) => {
    ...
    if (!comment.author.equals(req.user._id)) return res.status(403).json({ error: "Forbidden" });
    comment.deleteOne();
    await post.save();
    res.json({ success: true });
});
